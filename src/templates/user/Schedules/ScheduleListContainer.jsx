...
import { fetchQuoteForContainer } from "./info/operations";
...
const ScheduleListContainer = () => {
  ...
  useEffect(() => {
    fetchQuoteForContainer(location.search, setQuoteList);
  }, []);

  const toenterdetail = (item) => {
    const params = createToEnterDetail(item);
    const url = `/confirmdetailedinput/container?${params}`;
    dispatch(push(url));
  };
  ...
  return (
    <>
      <div className={classes.main_body_list}>
        ...
        <div className={classes.main_body_list_item_title_freetime}>F/T</div>
      ...
      {quoteList.length === 0 ? (
        ...
        quoteList.map((item, index) => (
          <div className={classes.main_body_list}>
            ...
            <div className={classes.main_body_list_item_freetime}>
              {item.freetime} Days
            </div>
