...
const OperatorBookingsDetailBoxPallet = () => {
  ...
  const data = [
    ...
    createData(
      "トラック配送",
      items.need_truck === true ? "依頼する" : "依頼しない"
    ),
    createData(
      "積載依頼",
      items.need_loading_or_unloading === true ? "依頼する" : "依頼しない"
    ),
    createData("作業場所", items.workplace_truck || "-"),
    createData("作業場所住所", items.street_address_truck || "-"),
    createData(
      "配送希望日時",
    ...
    createData("積載容量(M3)", items.cubic_meter),
    createData("サイズL", items.size_l),
    createData("サイズW", items.size_w),
    createData("サイズH", items.size_h),
    createData("重量", items.weight),
    createData("レベニュートン", items.revenue_ton),
    createData("パレットタイプ", items.pallet_type_id),
  ...
  const pricedata = [
    ...
    createPriceData("トラック料金", "¥" + truckbuy, "¥" + trucksell),
    createPriceData("積載料金", "¥" + truckloadingbuy, "¥" + truckloadingsell),
    createPriceData(
      "合計",
